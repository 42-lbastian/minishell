...
/*   Updated: 2022/12/12 15:50:07 by lbastian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ms_set_dup2(int pip[2][2], int type)
{
	if (type == CMD_BEGIN || type == CMD_MIDDLE || type == CMD_FILE_OUT
		|| type == CMD_FILE_OUT_END || type == CMD_FILE_IN || type == CMD_HERE_DOC)
		dup2(pip[1][1], STDOUT_FILENO);
	if (type == CMD_MIDDLE || type == CMD_END || type == CMD_FILE_OUT
		|| type == CMD_FILE_OUT_END || type == CMD_FILE_IN
		|| type == CMD_FILE_IN_END || type == CMD_ALONE || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
...
{
	if (type == CMD_MIDDLE || type == CMD_END || type == CMD_FILE_OUT
		|| type == CMD_FILE_OUT_END || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
		close(pip[0][0]);
	if (type == CMD_MIDDLE || type == CMD_END || type == CMD_FILE_OUT
		|| type == CMD_FILE_OUT_END || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
		close(pip[0][1]);
	if (type == CMD_BEGIN || type == CMD_MIDDLE || type == CMD_END
		|| type == CMD_ALONE || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
		close(pip[1][0]);
	if (type == CMD_BEGIN || type == CMD_MIDDLE || type == CMD_END
...
void	ms_close_fd_parent(int pip[2][2], int type)
{
	if (type == CMD_MIDDLE || type == CMD_END || type == CMD_FILE_OUT_END
		|| type == CMD_FILE_IN || type == CMD_FILE_IN_END || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
		close(pip[0][0]);
	if (type == CMD_MIDDLE || type == CMD_END || type == CMD_FILE_OUT_END || type == CMD_HERE_DOC_END || type == CMD_HERE_DOC)
		close(pip[0][1]);
