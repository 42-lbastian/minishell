...
/*   Updated: 2022/11/28 15:41:19 by lbastian         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ms_read_file_in_short(t_lst_parser *lst, t_env *st, int *pip)
{
	...
	read_lst_parser_short(lst->next, st, pip2, fd);
	return (EXIT_SUCCESS);
}

static int	ms_read_file_out_short(t_lst_parser *lst, t_env *st, int read, int write)
{
	int	fd;
...
static int	ms_read_pipe_short(t_lst_parser *lst, t_env *st, int read, int write)
{
	int	pip[2];
	int	fd_pipe;
...
int	ms_read_lst_parser(t_lst_parser *lst, t_env *st, int *pipe, int fd2)
{
	int	pip[2];
	...
	if (lst && lst->type == PIPE)
		if (ms_read_pipe_short(lst, st, read, write) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	if (lst && lst->type == FILE_IN)
		if (ms_read_file_in_short(lst, st, read, write) == EXIT_FAILURE)
			return (EXIT_FAILURE);
	if (lst && (lst->type == FILE_OUT_OVER || lst->type == FILE_OUT_APP))
		if (ms_read_file_out_short(lst, st, read, write) == EXIT_FAILURE)
			return (EXIT_FAILURE);
